...
export class App implements OnInit {
  ...
  addPrescriptionItem(): void {
    this.newPrescription.items.push({ medication_name: '', dosage: '', frequency: '', duration: '', instructions: '' });
  }

  removePrescriptionItem(index: number): void {
    if (this.newPrescription.items.length > 1) this.newPrescription.items.splice(index, 1);
  }

  createPrescription(): void {
    const patient = this.patients.find(item => item.id === Number(this.newPrescription.patient));
    const validItems = this.newPrescription.items.filter(item => item.medication_name.trim() && item.dosage.trim());
    
    if (!patient || !validItems.length) {
      this.showNotice('Choose a patient and add at least one medication.', 'error');
      return;
    }
    ...
    this.http.post<any>(`${this.apiUrl}/prescriptions/`, request).subscribe({
      next: prescription => {
        ...
        this.showNotice(`Prescription created for ${patient.name}.`, 'success');
      },
      error: error => this.showNotice(error.error?.detail || 'Unable to create the prescription.', 'error')
    });
  }
